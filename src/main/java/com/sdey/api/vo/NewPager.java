...
public class NewPager extends TagSupport {
	...
	public int doStartTag() throws JspException {
		if (pages > 0) {
			...
			outStr.append("\">");
			outStr.append("<i class=\"ace-icon fa fa-angle-double-left\"></i></a></li>");
			for (int i = 1; i <= pages; i++) {
				if (i == pageNum) {
					outStr.append("<li class=\"active\"><a href=").append(url).append("&pageNum=").append(i);
					...
					if (pageNum - i >= 4 && i != 1) // 只显示当前页前三个页码
					{
						...
						outStr.append("<li><a href=").append(url).append("&pageNum=").append(i);
						outStr.append(">").append(i).append("</a></li>");
					}
				}
			}
			...
			outStr.append("\">");
			outStr.append("<i class=\"ace-icon fa fa-angle-double-right\"></i></a></li>");
			///
			outStr.append(
					"<li class='disabled'><a href=\"javascript:;\"><i>");
